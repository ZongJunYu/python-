...
//	验证账号注册
	...
	$("#username").blur(function(){
		...
		if(user.test($("#username").val())){
			$(".usero").html('ok').css("background", "#FFFFFF");
			//纯数字
		    if(userpure.test($("#username").val())){
		    	$(".usero").html("可包含“_”、“-”，不能是纯数字").css("background", "#FFF5D9");
			}
		    else{
		    	yanzen1 = true;
		    }
	    ...
	    //为空
			...
			$(".usero").html("用户名不能为空").css("background", "#FFF5D9");
		}
		else{
			$(".usero").html("用户名必须在3-15个字符之间").css("background", "#FFF5D9");
		}
	})
	...
	$("#pad").blur(function(){
		...
		if($("#pad").val().length>6 && $("#pad").val().length<21){
			yanzen2 = true;
			$(".padero").html('ok').css("background", "#FFFFFF");
		}
		else if($("#pad").val()==""){
			$(".padero").html("密码不能为空").css("background", "#FFF5D9");
		}
		else{
			$(".padero").html("密码长度应在6-20个字符之间").css("background", "#FFF5D9");
		}
	})
	...
	$("#confirmpas").blur(function(){
		...
		if( $("#confirmpas").val()==$("#pad").val()){
			yanzen3 = true;
			$(".anewero").html('ok').css("background", "#FFFFFF");
		}
		else{
			$(".anewero").html("两次输入的密码不一致").css("background", "#FFF5D9");
		}
	})
	...
	$("#mailbox").blur(function(){
		...
		if(pattern.test($("#mailbox").val())){
			yanzen4 = true;
			$(".mailero").html('ok').css("background", "#FFFFFF");
		}
		else if($("#mailbox").val()==""){
			$(".mailero").html("邮箱不能为空").css("background", "#FFF5D9");
		}
		else{
			$(".mailero").html("这不是一个有效的电子邮箱").css("background", "#FFF5D9");
		}
	})
